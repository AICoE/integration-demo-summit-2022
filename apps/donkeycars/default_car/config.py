...
PCA9685_I2C_BUSNUM = (
    None  # None will auto detect, which is fine on the pi. But other platforms should specify the bus num.
)
...
DRIVE_TRAIN_TYPE = (
    "SERVO_ESC"  # SERVO_ESC|DC_STEER_THROTTLE|DC_TWO_WHEEL|DC_TWO_WHEEL_L298N|SERVO_HBRIDGE_PWM|PIGPIO_PWM|MM1|MOCK
)
...
MM_PER_TICK = (
    12.7625  # How much travel with a single tick, in mm. Roll you car a meter and divide total ticks measured by 1,000
)
...
MIN_DELTA = 0.0005  # early stop will want this much loss change before calling it improved.
...
PRUNE_EVAL_PERCENT_OF_DATASET = 0.05  # percent of dataset used to perform evaluation of model.
...
WEB_INIT_MODE = (
    "user"  # which control mode to start in. one of user|local_angle|local. Setting local will start in ai mode.
)

# JOYSTICK
USE_JOYSTICK_AS_DEFAULT = (
    False  # when starting the manage.py, when True, will not require a --js option to use the joystick
)
...
# SOMBRERO
HAVE_SOMBRERO = (
    False  # set to true when using the sombrero hat from the Donkeycar store. This will enable pwm on the hat.
)
...
LOGGING_LEVEL = (
    "INFO"  # (Python logging level) 'NOTSET' / 'DEBUG' / 'INFO' / 'WARNING' / 'ERROR' / 'FATAL' / 'CRITICAL'
)
LOGGING_FORMAT = (
    "%(message)s"  # (Python logging format - https://docs.python.org/3/library/logging.html#formatter-objects
)
...
AUTO_CREATE_NEW_TUB = (
    False  # create a new tub (tub_YY_MM_DD) directory when recording or append records to data directory directly
)
...
AI_LAUNCH_ENABLE_BUTTON = (
    "R2"  # this keypress will enable this boost. It must be enabled before each use to prevent accidental trigger.
)
